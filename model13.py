...
def test(k_fold = False):
    ...
    if k_fold:
        ...
        model = createModel()
        model.fit(traindata,y_train, validation_data=(testdata,y_test), epochs= 10)
...
def createModel():
    ...
    # model.add(Dense(25))
    model.add(LSTM(25, recurrent_activation="tanh", return_sequences=True, input_shape = (input_step, input_features)))
    model.add(Dropout(0.5, noise_shape=None, seed=None))
    model.add(LSTM(25, recurrent_activation="tanh",return_sequences=True))
    model.add(Dropout(0.5, noise_shape=None, seed=None))
    model.add(LSTM(25, recurrent_activation="tanh"))
    model.add(Dense(out_shape, recurrent_activation="tanh"))
